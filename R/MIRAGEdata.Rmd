...
summarized_saturations <- lipidomicsDF %>%
  ...
  summarize(
    class_sat_pmol_mg = sum(pmol_mg[sat_level_1 == 0 & (sat_level_2 == 0 | is.na(sat_level_2))], na.rm = TRUE),
    class_unsat1_pmol_mg = sum(pmol_mg[sat_level_1 == 1 & (sat_level_2 <= 1 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 == 0 & sat_level_2 == 1], na.rm = TRUE),
    class_unsat2_pmol_mg = sum(pmol_mg[sat_level_1 == 2 & (sat_level_2 <= 2 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 < 2 & sat_level_2 == 2], na.rm = TRUE),
        class_unsat3_pmol_mg = sum(pmol_mg[sat_level_1 == 3 & (sat_level_2 <= 3 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 < 3 & sat_level_2 == 3], na.rm = TRUE),
    class_unsat4_pmol_mg = sum(pmol_mg[sat_level_1 == 4 & (sat_level_2 <= 4 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 < 4 & sat_level_2 == 4], na.rm = TRUE),
      class_unsat5_pmol_mg = sum(pmol_mg[sat_level_1 == 5 & (sat_level_2 <= 5 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 < 5 & sat_level_2 == 5], na.rm = TRUE),
    class_unsat6_pmol_mg = sum(pmol_mg[sat_level_1 == 6 & (sat_level_2 <= 6 | is.na(sat_level_2))], na.rm = TRUE) +
      sum(pmol_mg[sat_level_1 < 6 & sat_level_2 == 6], na.rm = TRUE),
    
    class_no_ether_pmol_mg = sum(pmol_mg[ether_bond == 0], na.rm = TRUE),
    class_ether_pmol_mg = sum(pmol_mg[ether_bond == 1], na.rm = TRUE),
...
### Outlier handling
```{r}
# Make function to define and label outliers in dataset
define_outliers <- function(df, outlier_based_on, outlier_col_name){
  # Initialize outlier column
  df[[outlier_col_name]] <- NA

  # Loop through each unique combination of class, group, and time
  for (class in unique(df$lipidClass)){
     # Handle lipidComposition, ensuring NAs are treated as a unique group
    compositions <- unique(c(df$lipidComposition[df$lipidClass == class], NA))  # Include NA explicitly

    for (comp in compositions){
      for (group in c("HED", "DIWL", "WLEX")){
        for (time in c("Pre", "Post")){

          # Define indices based on whether composition is NA or not
          if (is.na(comp)) {
            indices <- which(df$lipidClass == class & 
                             is.na(df$lipidComposition) &
                             df$time == time & 
                             df$group == group)
          } else {
            indices <- which(df$lipidClass == class & 
                             df$lipidComposition == comp & 
                             df$time == time & 
                             df$group == group)
          }

          # Calculate the Q1, Q3, and IQR for the current subgroup
          Q1 <- quantile(df[[outlier_based_on]][indices], 0.25, na.rm = TRUE)
          Q3 <- quantile(df[[outlier_based_on]][indices], 0.75, na.rm = TRUE)
          IQR <- Q3 - Q1
          ...
          df[[outlier_col_name]][indices] <- df[[outlier_based_on]][indices] < lower_bound | df[[outlier_based_on]][indices] > upper_bound
        }
      }
    }
  }
...
lipidomicsDF <- define_outliers (df = lipidomicsDF,
                 outlier_based_on = "pmol_mg_log",
                 outlier_col_name = "outlier_log")
lipidomicsDF <- define_outliers (df = lipidomicsDF,
                 outlier_based_on = "pmol_mg",
...
## Pre-post
### Mixed model
Should be identical to how statistics are calculated for lipidclass.
  Data must be condensed to not include lipidComp, as this will give duplicates
...
# Saturated
result.lipid.pre_post_sat <- map(unique_lipid_classes, ~ mixedmodel_pre_post_sat(lipidomicsDF_saturation, "sat_proportion", .x))
# Unsaturated
result.lipid.pre_post_unsat <- map(unique_lipid_classes, ~ mixedmodel_pre_post_sat(lipidomicsDF_saturation, "unsat_proportion", .x))
# Mono unsaturated
result.lipid.pre_post_mono_unsat <- map(unique_lipid_classes, ~ mixedmodel_pre_post_sat(lipidomicsDF_saturation, "unsat_mono_proportion", .x))
# Poly unsaturated
result.lipid.pre_post_poly_unsat <- map(unique_lipid_classes, ~ mixedmodel_pre_post_sat(lipidomicsDF_saturation, "unsat_poly_proportion", .x))


# Naming the results list
names(result.lipid.pre_post_sat) <- unique_lipid_classes
names(result.lipid.pre_post_unsat) <- unique_lipid_classes
names(result.lipid.pre_post_mono_unsat) <- unique_lipid_classes
names(result.lipid.pre_post_poly_unsat) <- unique_lipid_classes
...
### Scatter
```{r}
plot_pre_post_sat <- function(lipidomicsDF_saturation, lipid_class, p_val_df, y_label, save_folder) {
...
mean_data_class <- filtered_data %>%
  group_by(group, time) %>%
  summarise(mean_unsat_proportion = mean(unsat_proportion, na.rm = TRUE), .groups = 'drop')
...
plot <- ggplot(filtered_data, aes(
    ...
    y = unsat_proportion, group = interaction(id, group)
  )) +
  ...
  geom_line(data = mean_data_class, aes(x = interaction(group, time, sep = " "), y = mean_unsat_proportion, group = group),color = "black", linewidth = 1, position = position_nudge(x = 0.03)) +
...
# Saturated
    plot_pre_post_sat(lipidomicsDF_saturation = lipidomicsDF_saturation, lipid_class = lipid_class, p_val_df = est_and_p_sat$p.vals, y_label = "Proportion of saturated lipids", save_folder = "saturated")
  # Unsaturated
  plot_pre_post_sat(lipidomicsDF_saturation = lipidomicsDF_saturation, lipid_class = lipid_class, p_val_df = est_and_p_unsat$p.vals, y_label = "Proportion of unsaturated lipids", save_folder = "unsaturated")
  # Monounsaturated
  plot_pre_post_sat(lipidomicsDF_saturation = lipidomicsDF_saturation, lipid_class = lipid_class, p_val_df = est_and_p_mono_unsat$p.vals, y_label = "Proportion of mono unsaturated lipids", save_folder = "unsaturated_mono")
  # polyunsaturated
  plot_pre_post_sat(lipidomicsDF_saturation = lipidomicsDF_saturation, lipid_class = lipid_class, p_val_df = est_and_p_poly_unsat$p.vals, y_label = "Proportion of poly unsaturated lipids", save_folder = "unsaturated_poly")
}
...
# Define lipid groups
lipids_groups <- list(
  gr_all = c("TAG", "1,2-DG", "1,3-DG", "AC", "PC", "PE_PC ratio", "PE", "SPM", "SPH", "Cer", "dhCer", "GluCer", "LacCer", "GalCer", "LPC", "LPE", "LPG", "LPI", "LPS", "PA", "PS", "PG", "PI")
)
...
# Filter lipidomics data
  ...
  lipidomics_wide$pp_change <- lipidomics_wide$proportion_of_Post - lipidomics_wide$proportion_of_Pre
...
# file_name_save <- paste0(save_variable_path, "correlation_values_ch_saturation.rds")
# 
 # Save the variables
 #saveRDS(correlation_values_ch_saturation, file = file_name_save)

# OR LOAD VARIABLES

# correlation_values_ch_saturation <- readRDS(file = file_name_save)
```
...
```{r}
vulcano_crossSection <- function(input_vars, save_sub_path, correlation_results, plot_title, save_folder){
...
vulc_plot <- ggplot(vulc_data_crossSection, aes(x = spearman_rho_all, y = -log10(p_value))) +
  ...
  labs(title = paste0(save_sub_path, " - Correlations at baseline - ", plot_title), x = "Spearman rho", y = "-log10 P-value")+
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
1
    # Save path
  save_path <- paste0( getwd(), "/outputs/saturation/cross section/vulcano", "/", save_folder,"/" ,save_sub_path, ".jpeg")
...
for (i in seq_along(mirage_crossSectional_vars)) {
    ...
    names_to_use <- unlist(lapply(mirage_crossSectional_vars[[i]], function(sublist) {
        ...
        if(length(sublist) >= 1) {
         ...
         vulcano_crossSection(input_vars = names_to_use,
...
savePathClassComp <- paste0(getwd(), "/outputs/correlations in changes/plot, correlations changes composition/")
                            

savePathClass <- paste0(getwd(), "/outputs/correlations in changes/plot, correlations changes class/")
...
# Save the plot
save_path <- paste0(getwd(), "/outputs/correlations in changes/vulcano/",sub_folder, "/", save_sub_path, ".jpg")
...
```{r}
# Save folder for .jpg output files
savePathClassComp <- paste0(getwd(), "/outputs/cross section/plot, correlations cross sectional composition/")
savePathClass <- paste0(getwd(), "/outputs/cross section/plot, correlations cross sectional class/")
...
```{r}
vulcano_crossSection <- function(input_vars, save_sub_path){
...
vulc_plot <- ggplot(vulc_data_crossSection, aes(x = spearman_rho_all, y = -log10(p_value))) +
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
    ...
    # Save path
  save_path <- paste0( getwd(), "/outputs/cross section/vulcano/", save_sub_path, ".jpeg")
...
for (i in seq_along(mirage_crossSectional_vars)) {
    ...
    names_to_use <- unlist(lapply(mirage_crossSectional_vars[[i]], function(sublist) {
        ...
        if(length(sublist) >= 1) {
         ...
         vulcano_crossSection(input_vars = names_to_use , save_sub_path = list_name)
}
...
# Save the plot
ggsave( paste0(getwd(), "/outputs/lipids, pre-post/vulcano/slope_volcano.jpg"), plot = vulc_plot, width = 8, height = 10, dpi = 400)
```
...
# Save the plot
ggsave( paste0( getwd(), "/outputs/lipids, pre-post/vulcano/prePost_volcano.jpg"), plot = vulc_plot, width = 8, height = 10, dpi = 300)
```
...
# Define the path and name of your PDF file
pdf_path <- paste0( getwd(), "/outputs/lipids, pre-post/heatmap/heatmap.pdf")
...
# Define output path
output_base_path <- paste0( getwd(), "/outputs/lipids, pre-post/plot, p-value sorted pre-post")
...
plot_lipid <- function(lipidomicsDF, lipid_composition, lipid_class, detection_limits, p_val_df, unit, save_subfolder) {
...
# Calculate means for each group and time point using dynamic variable
...
y_label_text <- switch(unit,
  ...
  plot <- ggplot(filtered_data, aes(
    ...
    # Black cross to signal outlier based on ULOQ
    geom_point(
      ...
      alpha = 0.5, position = position_nudge(x = 0.00), show.legend = FALSE 
        ...
        # Red cross to signal outlier based on ULOQ
    geom_point(
          data = subset(filtered_data, within_threshold == 3), aes(y = !!sym(unit)), color = "red", size = 4, shape = 4,
      alpha = 0.5, position = position_nudge(x = 0.00), show.legend = FALSE 
    ) +
    ...
    # Insert P-values pre post
    annotate("text",
      x = c(1.5, 3.5, 5.5), y = max(filtered_data[[unit]]) * 1.05,
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd(), "/outputs/lipids, pre-post/plot, scatter composition/", save_subfolder,"/",
...
for (lipid_class in unique_lipid_classes) {
  lipid_compositions_in_class <- unique(lipidomicsDF$lipidComposition[lipidomicsDF$lipidClass == lipid_class])
  for (lipid_composition in lipid_compositions_in_class) {
    plot_lipid(lipidomicsDF, lipid_composition, lipid_class, detection_limits, est_and_p_lipid_class_and_comp$p.vals, unit = "pmol_mg", save_subfolder = "non_transformed")
    plot_lipid(lipidomicsDF, lipid_composition, lipid_class, detection_limits, est_and_p_lipid_class_and_comp$p.vals, unit = "pmol_mg_log", save_subfolder = "log_transformed")
  }
}
...
```{r}
plot_lipid_class <- function(lipidomicsDF, lipid_class, detection_limits, p_val_df, unit, save_subfolder) {
  # Filter data based on lipid class and available time
...
plot <- ggplot(pmol_mg_class_sum, aes(
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd() ,"/outputs/lipids, pre-post/plot, scatter class/", save_subfolder, "/",
    gsub(" ", "_", lipid_class), ".jpg"
  )
  ggsave(filename, plot = plot, device = "jpeg", width = 8, height = 6, dpi = 300)
...
for (lipid_class in unique_lipid_classes) {
  plot_lipid_class(lipidomicsDF = lipidomicsDF,
                   ...
                   unit = "sum_pmol_mg",
                   save_subfolder = "non_transformed")
...
plot <- ggplot(filtered_data, aes(
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd(), "/outputs/lipids, pre-post/plot, scatter class, z_score/",
    gsub(" ", "_", lipid_class), ".jpg"
  )
  ggsave(filename, plot = plot, device = "jpeg", width = 8, height = 6, dpi = 300)
...
#### Based on everything grouped within lipid comp and class
```{r}
...
for (i in 1:nrow(unique_combinations)) {
  ...
  plot_mg_box <- ggplot(data = subset_lipidomicsDF, aes(x = factor(1), y = pmol_mg)) +
    geom_boxplot(outlier.shape = 4, outlier.size = 5) +
...
calculate_differences <- function(df) {
  ...
  combined$difference <- combined$pmol_mg.post - combined$pmol_mg.pre
  combined$difference_log <- log(combined$pmol_mg.post + 1) -  log(combined$pmol_mg.pre + 1)
  
  return(combined)
}

plot_differences <- function(df, class, comp, log_transform = FALSE) {
...
# Load data subset
class_comp <- lipidomicsDF[lipidomicsDF$lipidClass == "AC" & lipidomicsDF$lipidComposition == "18:0" & lipidomicsDF$time == "Pre" & lipidomicsDF$group == "HED",]

# Setting up the plotting area for two plots: 2 rows, 2 columns
par(mfrow = c(1, 2))

## Non-transformed
# Shapiro-Wilk Test for non-transformed data
shapiro_result_nonTransformed <- shapiro.test(class_comp$pmol_mg)

# Histogram for non-transformed data
#hist(class_comp$pmol_mg, main = sprintf("pmol_mg\nShapiro-Wilk: W = %.4f, p = %.3f", shapiro_result_nonTransformed$statistic, shapiro_result_nonTransformed$p.value), xlab = "Data points", breaks = 5)

# Q-Q plot for non-transformed data
qqnorm(class_comp$pmol_mg, main = sprintf("pmol_mg\nShapiro-Wilk: W = %.4f, p = %.3f", shapiro_result_nonTransformed$statistic, shapiro_result_nonTransformed$p.value), ylab = "Sample Quantiles")
...
hist(class_comp$pmol_mg_log, main = sprintf("pmol_mg_log\nShapiro-Wilk: W = %.4f, p = %.3f", shapiro_result_logTransformed$statistic, shapiro_result_logTransformed$p.value), xlab = "Data points", breaks = 5)

# Q-Q plot for log-transformed data
qqnorm(class_comp$pmol_mg_log, main = "Q-Q Plot: pmol_mg_log", ylab = "Sample Quantiles")
...
```{r}
output_dir <- paste0( getwd(), "/outputs/plot, qq-plots")
if (!dir.exists(output_dir)) {
...
for (i in 1:nrow(unique_combinations)) {
  ...
  # Compute log transformation for plotting (if necessary)
  subset_lipidomicsDF$pmol_mg_log <- log10(subset_lipidomicsDF$pmol_mg + 1)  # Adjust based on the data and needs
  
  # Plot for pmol_mg
  plot_mg <- ggplot(data = subset_lipidomicsDF, aes(x = factor(1), y = pmol_mg)) +
    geom_boxplot(outlier.shape = 4, outlier.size = 5) +
    geom_point(aes(color = id), position = position_jitter(width = 0.1), alpha = 0.6) +
    labs(y = "pmol_mg") +
    theme(axis.title.y = element_text(face = "bold", size = 14), axis.text.x = element_blank(), axis.ticks.x = element_blank())
  
  # Plot for pmol_mg_log
  plot_mg_log <- ggplot(data = subset_lipidomicsDF, aes(x = factor(1), y = pmol_mg_log)) +
    geom_boxplot(outlier.shape = 4, outlier.size = 5) +
    geom_point(aes(color = id), position = position_jitter(width = 0.1), alpha = 0.6) +
    labs(y = "pmol_mg_log") +
    theme(axis.title.y = element_text(face = "bold", size = 14), axis.text.x = element_blank(), axis.ticks.x = element_blank())
  ...
  combined_plot <- plot_mg / plot_mg_log +
                   plot_layout(guides = 'collect') +
                   plot_annotation(title = paste(class, comp), 
                                   theme = theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold")))

  # Define filename using sanitized strings
  sanitized_class_comp <- gsub("/", "_", gsub(":", ",", paste(class, comp)))
  filename <- paste0(output_dir, sanitized_class_comp, ".png")
  
  # Save the combined plot with sanitized filename
  ggsave(filename, plot = combined_plot, width = 10, height = 16, dpi = 72)
...
```{r}
output_dir <- paste0( getwd(), "/outputs/qq-plots/log transformed")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
