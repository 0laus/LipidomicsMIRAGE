...
library(foreach) # Run loops in parallel
...
# Outlier threshold: The factor to multiply by the IQR to define out¨liers to remove.
...
### Outlier handling
Outliers are defined based on two criteria:
1) Outside "outlier_IQR_factor" x the IQR for all measurements for a lipidcomposition within a lipidclass.
2) Outside "outlier_IQR_factor" x the IQR for the post - pre change for all measurements within lipidcomposition and lipidclass.
...
define_outliers <- function(df, value_col, outlier_col_name){
  ...
  dyn_difference_name <- paste0(value_col, "_PrePost_delta")
  ...
  difference_df <- lipidomicsDF %>%
    ...
    mutate(!!dyn_difference_name := Post - Pre) %>%
  ...
  for (class in unique(df$lipidClass)){
    for (comp in unique(df$lipidComposition)){
      ...
      median_value <- median(df[[value_col]][df$lipidClass == class & df$lipidComposition == comp])
      ...
      Q1_change <- quantile(df[[dyn_difference_name]][indices], 0.25, na.rm = TRUE)
      Q3_change <- quantile(df[[dyn_difference_name]][indices], 0.75, na.rm = TRUE)
      ...
      lower_bound_change <- Q1 - outlier_IQR_factor * IQR
      upper_bound_change <- Q3 + outlier_IQR_factor * IQR
      
      
      # Update the 'outlier' column based on calculated bounds
      df[[composition_outlier_name]][indices] <- df[[value_col]][indices] < lower_bound | df[[value_col]][indices] > upper_bound
      
      df[[change_outlier_name]][indices] <- df[[dyn_difference_name]][indices] < lower_bound_change | df[[dyn_difference_name]][indices] > upper_bound_change
...
**Add PE/PC ratio**
Outliers are handled by calculating from imputed values as the PE and PC classes are summed.
```{r}
...
# Calculate the PE/PC ratio
ratios_df <- sums_wide %>%
  mutate(lipidClass = "PE_PC",
...
**Add Cer 16:0 / Cer 24:0 ratio**
Outliers are handled by filtering out outlier data
```{r}
add_ratio_lipidomics <- function (lipidomicsDF)
{
  
  workingDF <- lipidomicsDF[lipidomicsDF$outlier == FALSE,] %>%
...
workingDF <- workingDF %>%
  group_by(id, time, group) %>%  # Remove lipidComposition from the grouping
  mutate(
  ...
  ungroup()  # Ensure the grouping is removed after updating
...
wideDF <- workingDF %>%
  ...
  pivot_wider(
    ...
    values_from = pmol_mg,
    values_fill = list(pmol_mg = NA)  # This fills only where actual data is missing
  )

# Calculate the ratio
ratios_df <- wideDF %>%
  mutate(lipidClass = "Cer16_Cer24",
         lipidComposition = "ratio",
         pmol = NA_real_,
         pmol_mg = `16:0` / `24:0`,
         within_threshold = NA) %>%
...
result.lipid.pre_post.class_comp.outlierRM <- pmap(
  list(lipidClass = unique_combinations$lipidClass, lipidComposition = unique_combinations$lipidComposition),
  ~ mixedmodel_pre_post(lipidomicsDF[lipidomicsDF$outlier == FALSE,], "pmol_mg", ..1, ..2)
)
...
names(result.lipid.pre_post.class_comp.outlierRM) <- paste(unique_combinations$lipidClass, unique_combinations$lipidComposition, sep = "_")


# Use map to iterate over lipid classes
result.lipid.pre_post.class <- map(unique_lipid_classes, ~ mixedmodel_pre_post(lipidomicsDF, "pmol_mg", .x))
# With outliers imputed
result.lipid.pre_post.class.outlierRM <- map(unique_lipid_classes, ~ mixedmodel_pre_post(lipidomicsDF, "pmol_mg_imp", .x))

# Naming the results list
names(result.lipid.pre_post.class) <- unique_lipid_classes
names(result.lipid.pre_post.class.outlierRM) <- unique_lipid_classes
...
```{r}
pvals_df <- est_and_p_lipid_class_and_comp.outlierRM$p.vals
estimates_df <- est_and_p_lipid_class_and_comp.outlierRM$estimates
...
# Save the plot
ggsave( paste0(getwd(), "/outputs/pre_post/vulcano/slope_volcano.jpg"), plot = vulc_plot, width = 8, height = 10, dpi = 400)
```
...
# Save the plot
ggsave( paste0( getwd(), "/outputs/pre_post/vulcano/prePost_volcano.jpg"), plot = vulc_plot, width = 8, height = 10, dpi = 300)
```
...
y_label_text <- switch(unit,
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd(), "/outputs/pre_post/plot, scatter composition/", save_subfolder,"/",
...
for (lipid_class in unique_lipid_classes) {
  ...
  for (lipid_composition in lipid_compositions_in_class) {
    plot_lipid(lipidomicsDF = lipidomicsDF,
               ...
               p_val_df_outlierRM = est_and_p_lipid_class_and_comp.outlierRM$p.vals,
               unit = "pmol_mg",
...
plot <- ggplot(pmol_mg_class_sum, aes(
  ...
  labs(title = lipid_class, 
       ...
       y = y_label,
       x = "") +
  annotate("text", x = c(1.5, 3.5, 5.5), y = 3.0, label = annotation_pvals_pre_post, size = 1.75) +
  theme_minimal() +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "right",
        legend.title.align = 0.5, plot.title = element_text(hjust = 0.5), legend.text = element_text(size = 6),
        legend.title = element_text(size = 6), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd() ,"/outputs/pre_post/plot, scatter class/", save_subfolder, "/",
    gsub(" ", "_", lipid_class), ".jpg"
  )
  ggsave(filename, plot = plot, device = "jpeg", width = 8, height = 6, dpi = 300)
...
# Loop to call the plotting function for each lipid class
for (lipid_class in unique_lipid_classes) {
  plot_lipid_class(lipidomicsDF = lipidomicsDF[lipidomicsDF$outlier==FALSE,],
                   lipid_class = lipid_class,
                   detection_limits = detection_limits,
                   p_val_df = est_and_p_lipid_class_and_comp$p.vals,
                   p_val_df.outlierRM = est_and_p_lipid_class_and_comp.outlierRM$p.vals,
                   unit = "sum_pmol_mg",
                   save_subfolder = "non_transformed")
...
for (i in 1:nrow(unique_combinations)) {
  ...
  png(file.path(output_dir, paste0(gsub("[/:]", "_", paste(class, comp)), ".png")), width = 1600, height = 1200)
  par(mfrow = c(6, 2))
  ...
  for (group in c("HED", "DIWL", "WLEX")){
    for (time in c("Pre", "Post")){
      ...
      if (length(class_comp$pmol_mg_log) >= 3 && length(unique(class_comp$pmol_mg_log)) > 1) {
      ...
      if (length(class_comp$pmol_mg_log) >= 3) {  # Ensure enough data for qqplot
