...
physicalActivity <- c("daily_steps_steps_24hr_pre", "tdpa_pre", "vo2abs_pre", "vo2kg_ffm_pre", "x4m_walk_pre", "step_test_pre", "x1rm_pre", "age_pre")
...
if (plot_lipids[i] %in% c("TAG", "PE", "PC")) {
  ...
  x_label <- bquote(Delta * " " * nmol %.% mg^{-1})
    ...
    plot <- ggplot(correlation_df, aes(x = pmol_mg_ch_id, y = !!sym(var_name_ch), color = group)) +
      geom_point(size = .8) +
...
ggsave(paste0(getwd(), "/outputs/figures/figure 2/", save_name, ".svg"), plot_grid, device = "pdf", height = save_size[1], width = save_size[2] , units = "mm")
}

# Selected clinical  
clinical_vars <- list(
  c("rd_ss2kg_ffm_insulin_pre", "rd_ss2kg_ffm_insulin_post", bquote(atop(NA, atop(textstyle(bold(GIR/I)), textstyle(Delta * " " * "mg/kg/min/ins"))))),
  c("hgb_a1c_pre", "hgb_a1c_post", bquote(atop(NA, atop(textstyle(bold(HbA1c)), textstyle(Delta * " " * "%"))))),
  c("vo2kg_ffm_pre", "vo2kg_ffm_post", bquote(atop(NA, atop(textstyle(bold(VO[2] * " Peak")), textstyle(Delta * " " * "ml/kgFFM"))))),
  c("tdpa_pre", "tdpa_post", bquote(atop(NA, atop(textstyle(bold("Physical Activity")), textstyle(Delta * " " * "min"))))),
  c("percent_imat_pre", "percent_imat_post", bquote(atop(NA, atop(textstyle(bold(IMAT)), textstyle(Delta * " " * "%")))))
...
TAG_plot_lipids <- c("TAG","1,2-DG", "1,3-DG")
TAG_plot_colNames <- c("TAG", "1,2-DAG", "1,3-DAG")
