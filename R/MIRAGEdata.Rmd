...
```{r}
vulcano_changes <- function(input_vars, save_sub_path, spearman_types_select){
...
vulc_plot <- ggplot(vulc_data, aes(x = rho_value, y = -log10(p_value))) +
  ...
  theme_minimal() +
  labs(title = paste0("Correlation: ", save_sub_path),
...
# Save the plot
save_path <- paste0("/Users/au309166/Library/CloudStorage/OneDrive-Aarhusuniversitet/Forskning/Projects/LipidomicsMIRAGE/outputs/correlations in changes/vulcano/", save_sub_path, ".jpg")
...
for (i in seq_along(mirage_ch_vars)) {
    ...
    vulcano_changes(input_vars = names_to_use, save_sub_path = paste0("all/", list_name), spearman_types_select = spearman_types_select.all)
    vulcano_changes(input_vars = names_to_use, save_sub_path = paste0("groups/", list_name), spearman_types_select = spearman_types_select.groups)
}
# Add groups_to_plot
```

### Heatmap CORRECT TO MAKE FOR EVERY GROUP!
```{r}
...
long_format_p_adj_groups.all <- lapply(lipids_groups, filter_lipid_group_base, data_frame = long_format_p_adj.gr.class)
# correlations
...
mean_data_comp <- filtered_data %>%
  ...
  plot <- ggplot(filtered_data, aes(
    ...
    geom_point(
          data = subset(filtered_data, within_threshold == 3), aes(y = pmol_mg), size = 7, shape = 4, color = "black", fill = "grey",
      alpha = 0.5, position = position_nudge(x = 0.03) # cross to signal outlier
    ) +
