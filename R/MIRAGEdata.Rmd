...
** Add LPC/PC ratio ** Ref: doi: 10.1172/JCI135963
```{r}
add_ratio_lipidomics <- function (lipidomicsDF)
{
  
  # Filter for LPC and PC lipid classes and summarize their pmol values
  sums_df <- lipidomicsDF %>%
    filter(lipidClass %in% c("LPC", "PC")) %>%
    group_by(id, time, sampleNumber, group, wet_weight, lyphilized_weight, volume_of_homogenate_ul, equiv_weight_extracted, lipidClass) %>%
    summarise(
      sum_pmol_mg = sum(pmol_mg_imp),
      .groups = "drop"
  ...
  # Calculate the PC/PE ratio
  ratios_df <- sums_wide %>%
    mutate(lipidClass = "LPC.PC",
           lipidComposition = "ratio",
           pmol = NA_real_,
           pmol_mg = LPC / PC,
           within_threshold = NA) %>%
    select(-LPC, -PC)# Remove intermediate PE and PC columns
...
bodycomposition <- c("bmi_pre", "percent_imat_pre", "bmi_post", "percent_imat_post", "thigh_imat_g_pre", "thigh_imat_g_post", "percent_fat_mass_pre", "percent_fat_mass_post")
...
unique_lipid_classes.noRatios <- unique_lipid_classes[!unique_lipid_classes == c("PC.PE", "LPC.PC", "Cer ratio")]
...
heatmap_pre_post_sat <- function(result_data, overall_title){
  ...
  # Define lipid groups
  lipids_groups <- list(
    gr_all = c("TAG", "1,2-DG", "1,3-DG", "AC", "PC", "PC.PE", "LPC.PC", "PE", "SPM", "SPH", "Cer", "dhCer", "GluCer", "LacCer", "GalCer", "LPC", "LPE", "LPG", "LPI", "LPS", "PA", "PS", "PG", "PI")
  )
...
for (saturation_type in c("sat_proportion", "unsat_proportion", "unsat_mono_proportion", "unsat_poly_proportion")){
  ...
  for (lipid_class in unique_lipid_classes[unique_lipid_classes != c("PC.PE", "LPC.PC", "Cer ratio")]) {
...
# Within mirage, pre-post changes
```{r}
  ##### HOW MUCH OF WEIGHT LOSS IS FAT LOSS
  # Turn MIRAGE data into long format to be able to plot data
  # Combine all variables of interest
  endpoint_variable <- c("weight_pre", "weight_post", "fat_mass_dxa_pre", "fat_mass_dxa_post")
  ...
  mirage_subset <- mirage_subset %>%
    mutate(in_lipidomics = if_else(id %in% lipidomicsDF$id, TRUE, FALSE))
  
  
  # Calculate the differences and fat_loss_percent
mirage_subset <- mirage_subset %>%
  mutate(
    weight_loss = weight_pre - weight_post,
    fat_loss = fat_mass_dxa_pre - fat_mass_dxa_post,
    fat_loss_percent = fat_loss / weight_loss * 100
  )

View(mirage_subset)

# Calculate mean and standard deviation for each group
summary_stats <- mirage_subset %>%
  group_by(group) %>%
  filter(in_lipidomics == TRUE) %>% 
  summarize(
    mean_fat_loss_percent = mean(fat_loss_percent, na.rm = TRUE),
    sd_fat_loss_percent = sd(fat_loss_percent, na.rm = TRUE)
  )
...
summary_stats
  ...
  ##### VO2 / kg / min
   # Turn MIRAGE data into long format to be able to plot data
  # Combine all variables of interest
  endpoint_variable <- c("vo2abs_pre", "vo2abs_post", "weight_pre", "weight_post")
...
mirage_subset <- mirage_subset %>%
  mutate(
    vo2_kg_pre = vo2abs_pre / weight_pre * 1000,
    vo2_kg_post = vo2abs_post / weight_post * 1000,
  )
...
summary_stats <- mirage_subset %>%
  ...
  filter(in_lipidomics == TRUE) %>% 
  summarize(
    mean_vo2_kg_pre = mean(vo2_kg_pre, na.rm = TRUE),
    sd_vo2_kg_pre = sd(vo2_kg_pre, na.rm = TRUE)
  )
...
lipidomicsDF_filtered <- summarizedDF %>%
  filter(!(lipidClass %in% c("Cer ratio", "PC.PE", "LPC.PC"))) %>%
...
mirage_included <- mirage_data %>%
  ...
  mutate(
    ...
    thigh_imat_g_change = (thigh_imat_g_post - thigh_imat_g_pre) / thigh_imat_g_pre * 100,
...
label(mirage_included$tdpa_pre) <- "ᶜ Physical activity"
...
label(mirage_included$thigh_muscle_g_pre) <- "ᵇ Thigh muscle"
label(mirage_included$thigh_imat_g_pre) <- "ᵇ Thigh IMAT"
...
units(mirage_included$thigh_imat_g_pre) <- "g"
...
# Footnote
footnote <-paste0 ("ᵃ Rate of glucose disposal (R<sub>d</sub>) normalized to fat-free mass and blood insulin. n: DIWL = 9, WLEX = 8, HED = 7<br/>", # <br/> used as linebreak for html table
                   "ᵇ Values obtained from magnetic resonance imaging n: DIWL = 6, WLEX = 11, HED = 8<br/>",
                   # "ᵇ Intramuscular adipose tissue<br/>",
                   "ᶜ Minutes of daily physical activity above 1.5 metabolic equivalent of task (MET)")
...
result_table <- table1(~ + age_pre +
                         sex_0_male_1_female  +
                         t2d_status + rd_ss2kg_ffm_insulin_pre +
                         hgb_a1c_pre + 
                         bmi_pre +
                         weight_pre + 
                         fat_mass_dxa_pre +
...
unique_lipid_classes <- unique(lipidomicsDF$lipidClass)
for (lipid_class in unique_lipid_classes[!unique_lipid_classes %in% c("PC.PE", "LPC.PC")]) {
...
corr_ordered <- correlation_values_ch_class %>%
  ...
  mutate(lipid = ifelse(lipid == "PC.PE", "PC/PE", 
                        ifelse(lipid == "LPC.PC", "LPC/PC", lipid))) %>%
...
clinical_var <- list(
  ...
  c("thigh_imat_g_ch", bquote(bold("Thigh IMAT")), .7),
...
if (is.na(hm_matrix[i, j])) {
  ...
  if (p_val_matrix[i, j] < pLims$p1) {
    grid.text("*\n*\n*", unit(x, "npc"), unit(y, "npc") - unit(0.07, "npc"), gp = gpar(col = "black", fontsize = 5, fontface = "bold", lineheight = 0.4), just = "center")
  } else if (p_val_matrix[i, j] < pLims$p2) {
    grid.text("*\n*", unit(x, "npc"), unit(y, "npc") - unit(0.07, "npc"), gp = gpar(col = "black", fontsize = 5, fontface = "bold", lineheight = 0.4), just = "center")
  } else if (p_val_matrix[i, j] < pLims$p3) {
    grid.text("*", unit(x, "npc"), unit(y, "npc") - unit(0.07, "npc"), gp = gpar(col = "black", fontsize = 5, fontface = "bold", lineheight = 0.4), just = "center")
                  }
            }
          })
...
lipids_groups <- list(
  ...
  gr_lpc_pc = c("LPC.PC", "LysoPhosphatidylcholine.Phosphatidylcholine", 0.85, 5, FALSE, "row_title"),
...
clinical_var <- list(
  ...
  c("thigh_imat_g_ch", bquote("Thigh IMAT"), .7),
...
if (plot_lipids[i] %in% c("TAG", "1,2-DAG", "1,3-DAG", "PE", "PC", "PI")) {
  ...
  x_label <- bquote(Delta * " " * nmol %.% mg^{-1})
    ...
    # Make PC/PE percentage point
        if (plot_lipids[i] %in% c("PC.PE", "LPC.PC", "Cer ratio")) {
...
Lysophospholipids_plot_lipids <- c("LPC", "LPE", "LPG", "LPI", "LPS", "LPC.PC")
Lysophospholipids_plot_colNames <- c("LPC", "LPE", "LPG", "LPI", "LPS", "LPC/PC")
...
heatmap_changes <- function(test_against_input, save_name)
{
  ...
  lipids_groups <- list(
    ...
    gr_pc = c("PC", "PC.PE", "LPC.PC"),  # Phosphatidylcholines and ratio
  ...
  # Make Na if too few values determined from prepare data function
    ...
    for (i in seq_along(mirage_variables_of_interest)) {
      ...
      for (j in seq_along(mirage_variables_of_interest[[i]])){
        ...
        # Print n for each lipid class
        if ( is.null(lipidcomposition) && lipidclass == "AC"){
          n_correlations <- sum(!is.na(corr_df$lipidClass == "AC") & corr_df$lipidComposition == corr_df$lipidComposition[!is.na(corr_df$lipidComposition)][1] & !is.na(corr_df[[mirage_variables_of_interest[[i]][[j]][1]]]))
          print(mirage_variables_of_interest[[i]][[j]][1])
          print ( paste0 ( "n = " , n_correlations))
        }
        ...
        sum(!is.na(corr_df$lipidClass) & !is.na(corr_df[[test_against_name]]))
...
clinical_var <- list(
  ...
  c("tdpa_pre", bquote(bold("Daily physical activity")), .7),
  c("rd_ss2kg_ffm_insulin_pre", bquote(bold("Glucose R"[d])), .7),
  c("percent_fat_mass_pre", bquote(bold("Body fat proportion")), .7),
  c("percent_imat_pre", bquote(bold("Thigh IMAT proportion")), .7),
  c("thigh_imat_g_pre", bquote(bold("Thigh IMAT")), .7),
...
clinical_var_post <- list(
  ...
  c("tdpa_post", bquote(bold("Daily physical activity (post)")), .7),
  c("rd_ss2kg_ffm_insulin_post", bquote(bold("Glucose R"[d]~"(post)")), .7),
  c("percent_fat_mass_post", bquote(bold("Body fat proportion")), .7),
  c("percent_imat_post", bquote(bold("Thigh IMAT proportion (post)")), .7),
  c("thigh_imat_g_post", bquote(bold("Thigh IMAT (post)")), .7),
...
for (var in clinical_var){
  correlation_waterfall(clinical_var = var, 
                        save_sub_folder = "")
}
...
clinical_var <- list(
  ...
  c("thigh_imat_g_pre", bquote("Thigh IMAT"), .7),
...
clinical_var_post <- list(
  ...
  c("thigh_imat_g_post", bquote("Thigh IMAT (post)"), .7),
...
# Programmatically generate the "post" clinical variables
clinical_vars_post <- lapply(clinical_vars, function(x) {
  c(gsub("_pre", "_post", x[[1]]), x[[2]])
})
...
# Define plot_lipids and col_titles for Sphingolipids (first and second rows combined)
Sphingolipids_plot_lipids <- c("Cer", "dhCer", "GluCer", "LacCer", "SPH", "SPM")
Sphingolipids_plot_colNames <- c("Cer", "dhCer", "GluCer", "LacCer", "SPH", "SPM")
...
lipid_classes_per_comp <- unique_combinations$lipidClass[!unique_combinations$lipidClass %in% c("PC.PE", "LPC.PC")]

lipid_compositions <- unique_combinations$lipidComposition[!unique_combinations$lipidClass %in% c("PC.PE", "LPC.PC")]
...
y.labels <- list(
  "TAG" = bquote("Triacylglycerol (nmol · mg dry wt"^-1*")"),
  "1,2-DG" = bquote("1,2-diacylglycerol (pmol · mg dry wt"^-1*")"),
  "1,3-DG" = bquote("1,3-diacylglycerol (pmol · mg dry wt"^-1*")"),
  
  "AC" = bquote("Acylcarnitine (pmol · mg dry wt"^-1*")"),
  
  "Cer" = bquote("Ceramide (pmol · mg dry wt"^-1*")"),
  "Cer ratio" = bquote("Ceramide 16:0 / ceramide 24:0"),
  "dhCer" = bquote("Dihydroceramide (pmol · mg dry wt"^-1*")"),
  
  "GluCer" = bquote("Glucosylceramide (pmol · mg dry wt"^-1*")"),
  "LacCer" = bquote("Lactosylceramide (pmol · mg dry wt"^-1*")"),
  "SPH" = bquote("Sphingosine (pmol · mg dry wt"^-1*")"),
  "SPM" = bquote("Sphingomyelin (pmol · mg dry wt"^-1*")"),
  
  "GalCer" = bquote("Galactosylceramide (pmol · mg dry wt"^-1*")"),
  
  
  "LPC" = bquote("Lysophosphatidylcholine (pmol · mg dry wt"^-1*")"),
  "LPE" = bquote("Lysophosphatidylethanolamine (pmol · mg dry wt"^-1*")"),
  "LPG" = bquote("Lysophosphatidylglycerol (pmol · mg dry wt"^-1*")"),
  "LPI" = bquote("Lysophosphatidylinositol (pmol · mg dry wt"^-1*")"),
  "LPS" = bquote("Lysophosphatidylserine (pmol · mg dry wt"^-1*")"),
  "PA" = bquote("Phosphatidic acid (pmol · mg dry wt"^-1*")"),
  "PC" = bquote("Phosphatidylcholine (nmol · mg dry wt"^-1*")"),
  "PE" = bquote("Phosphatidylethanolamine (nmol · mg dry wt"^-1*")"),
  "PG" = bquote("Phosphatidylglycerol (pmol · mg dry wt"^-1*")"),
  "PI" = bquote("Phosphatidylinositol (pmol · mg dry wt"^-1*")"),
  "PS" = bquote("Phosphatidylserine (pmol · mg dry wt"^-1*")"),
  
  "PC.PE" = bquote("Phosphatidylcholine / phosphoethanolamine"),
  "LPC.PC" = bquote("Lysophosphatidylcholine / phosphatidylcholine")
)
...
ggsave(filename, plot = plot_ratios, device = device_out, width = 4, height = 3)
...
pvals_df <- rbind(
  est_and_p_lipid_comp.outlierRM$p.vals,
  est_and_p_pvals_lipid_class.outlierRM$p.vals
)

estimates_df <- rbind(
  est_and_p_lipid_comp.outlierRM$estimates,
  est_and_p_pvals_lipid_class.outlierRM$estimates
)

pvals_df <- pvals_df[order(pvals_df$lipid), ]
estimates_df <- estimates_df[order(estimates_df$lipid), ]
...
pvals_df <- pvals_df %>%
  rename(
    ...
    `group HED` = `Pre HED - Post HED (corrected)`,
  )
...
hedAdditionalHeigt <- 0.1
lipids_groups_full_names <- list(
  gr_tag_ac = c("Triacylglycerol", .7+hedAdditionalHeigt, 6, FALSE, "row_title"),
  gr_1_2_dg = c("1,2-diacylglycerol", .85+hedAdditionalHeigt, 6, TRUE,"row_title"),
  gr_1_3_dg = c("1,3-diacylglycerol", .85+hedAdditionalHeigt, 6, FALSE, "row_title"),
  gr_ac = c("Acylcarnitine", 0.7+hedAdditionalHeigt, 2.5, FALSE, "row_title"),
  
  gr_cer = c("Ceramide", .9+hedAdditionalHeigt, 3, FALSE, "row_title"),
  gr_cer_ratio = c("Ceramide ratio", 1+hedAdditionalHeigt, 3, TRUE, "row_title"),
  gr_dhcer = c("Dihydroceramide", .9+hedAdditionalHeigt, 3, TRUE,"row_title"),
  
  gr_glucer = c("Glucosylceramide", .9+hedAdditionalHeigt, 2, FALSE, "row_title"),
  gr_laccer = c("Lactosylceramide", .9+hedAdditionalHeigt, 2, FALSE, "row_title"),
  gr_sph = c("Sphingosine", .9+hedAdditionalHeigt, 2, FALSE, "row_title"),
  gr_spm = c("Sphingomyelin", .9+hedAdditionalHeigt, 2, FALSE, "row_title"),
  
  
  gr_pc = c("Phosphatidylcholine", 1.05+hedAdditionalHeigt, 6, TRUE,"row_title"),
  gr_pe = c("Phosphatidylethanolamine", 1.12+hedAdditionalHeigt, 6, FALSE,"row_title"),
  gr_pc_pe = c("Phosphatidylcholine.Phosphatidylethanolamine", 1+hedAdditionalHeigt, 6, FALSE,"row_title"),
  gr_lpc_pe = c("Lysophosphatidylcholine.Phosphatidylcholine", 1+hedAdditionalHeigt, 6, FALSE,"row_title"),
  
  gr_pa = c("Phosphatidic acid", .9+hedAdditionalHeigt, 3, FALSE,"row_title"),
  gr_pg = c("Phosphatidylglycerol", .9+hedAdditionalHeigt, 3, FALSE,"row_title"),
  gr_ps = c("Phosphatidylserine", .9+hedAdditionalHeigt, 3, FALSE,"row_title"),
  gr_pi = c("Phosphatidylinositol", .9+hedAdditionalHeigt, 3, FALSE,"row_title"),
  
  
  gr_lpc = c("Lysophosphatidylcholine", 1+hedAdditionalHeigt, 1.5, FALSE,"col_title"),
  gr_lpe = c("Lysophosphatidylethanolamine", 1+hedAdditionalHeigt, 3, TRUE,"col_title"),
  gr_lpg = c("Lysophosphatidylglycerol", 1+hedAdditionalHeigt, 1.5, FALSE,"col_title"),
  gr_lpi = c("Lysophosphatidylinositol", 1+hedAdditionalHeigt, 1.5, FALSE,"col_title"),
  gr_lps = c("Lysophosphatidylserine", 1+hedAdditionalHeigt, 1.5, FALSE,"col_title")
)
...
# Rename first row to Total
estimates_gr_list <- lapply(estimates_gr_list, function(df) {
  rownames(df)[1] <- "Total"
  df
})

pvals_gr_list <- lapply(pvals_gr_list, function(df) {
  rownames(df)[1] <- "Total"
  df
})
...
generate_heatmap <- function(hm_matrix,
                             p_val_matrix,
                             colorMap,
                             pLims,
                             hm_title,            # still here so your calls don’t break
                             hm_unit,
                             rowSeperatorLine = NULL) {

  # 1) clean up column names
  colnames(hm_matrix)    <- sub(".*?_", "", colnames(hm_matrix))
  colnames(p_val_matrix) <- colnames(hm_matrix)

  # 2) capture original names
  orig_cn <- colnames(hm_matrix)
  n_cols  <- length(orig_cn)

  # 3) build per-column gp:
  base_fs <- 5              # your original font size
  fs_vec  <- c(base_fs * 1.3, rep(base_fs, n_cols - 1))
  face_vec<- c("bold",     rep("plain", n_cols - 1))
  ...
  Heatmap(
    ...
    # ── here’s the per-column styling ───────────────────────────
    column_labels     = orig_cn,       
    column_names_rot  = 90,
    column_names_gp   = gpar(fontsize = fs_vec,
                             fontface = face_vec),
    ...
    heatmap_legend_param = list(
      ...
      color_key_height = unit(0.3, "cm"),
      color_key_width  = unit(0.8, "cm"),
    ...
    cell_fun = function(j, i, x, y, width, height, fill) {
      ...
      # p-value stars
        ...
        stars <- NULL
        if      (p_val_matrix[i, j] < pLims$p1) stars <- "***"
        else if (p_val_matrix[i, j] < pLims$p2) stars <- "**"
        else if (p_val_matrix[i, j] < pLims$p3) stars <- "*"
        if (!is.null(stars)) {
          grid.text(stars,
                    unit(x, "npc"), unit(y, "npc") - unit(0.07, "npc"),
                    gp = gpar(col = "black", fontsize = 6),
                    just = "center")
        }
      }
      # horizontal separatorline
      if (!is.null(rowSeperatorLine) && i == rowSeperatorLine) {
        grid.lines(x = c(x - width/2, x + width/2),
                   y = c(y - height/2, y - height/2),
                   gp = gpar(col = "black", lwd = 0.5))
      }
      # vertical gap line after col1
      if (j == 1) {
        grid.lines(
          x = c(x + width/2, x + width/2),
          y = c(y - height/2, y + height/2),
          default.units = "npc",
...
for (i in 1:length(lipids_groups)){
  ...
  hm_pre_post <- generate_heatmap(t(matrices_estimates[[i]][,c(1,2,4), drop = FALSE]),
                                  t(matrices_pvals[[i]][,c(1,2,4), drop = FALSE]),
                                  color_mapping_common,
                                  p_value_limits,
                                  hm_title = "",
                                  hm_unit = "Percent",
...
unique_lipid_classes <- unique(lipidomicsDF$lipidClass[!lipidomicsDF$lipidClass %in% c("PC.PE", "LPC.PC")])
...
plot_lipid_class <- function(lipidomicsDF, lipid_class, detection_limits, p_val_df, p_val_df.outlierRM, unit, save_subfolder) {
  ...
  else if("LPC.PC" %in% filtered_data$lipidClass) {
    y_label <- "LPC/PC ratio"
  }
  else {
    y_label <- switch(unit,
