...
Outliers are defined based on two criteria: 1) Outside "outlier_IQR_factor" x the IQR for all measurements for a lipidcomposition within a lipidclass. 2) Outside "outlier_IQR_factor" x the IQR for the post - pre change for all measurements within lipidcomposition and lipidclass.
...
define_outliers <- function(df, value_col, outlier_col_name){
  ...
  for (class in unique(df$lipidClass)){
    for (comp in unique(df$lipidComposition)){
      ...
      lower_bound_change <- Q1 - (outlier_IQR_factor * IQR_change)
      upper_bound_change <- Q3 + (outlier_IQR_factor * IQR_change)
...
**Add PE/PC ratio**
Outliers are handled by calculating from imputed values as the PE and PC classes are summed. As they are analysed on a lipid class level, stating outlier status is not performed.
...
Outliers are handled by filtering out outlier data in the function below
...
unique_lipid_classes.noRatios <- unique_lipid_classes[!unique_lipid_classes == c("PE.PC", "Cer ratio")]
...
# Define lipid groups
lipids_groups <- list(
  gr_all = c("TAG", "1,2-DG", "1,3-DG", "AC", "PC", "PE.PC ratio", "PE", "SPM", "SPH", "Cer", "dhCer", "GluCer", "LacCer", "GalCer", "LPC", "LPE", "LPG", "LPI", "LPS", "PA", "PS", "PG", "PI")
)
...
for (saturation_type in c("sat_proportion", "unsat_proportion", "unsat_mono_proportion", "unsat_poly_proportion")){
 ...
 for (lipid_class in unique_lipid_classes[unique_lipid_classes != c("PE.PC", "Cer ratio")]) {
...
Ouliers handling:
pmol_mg imputed is used. For composition, only rows that are not outliers are fed to the functions, i.e. imputed col will be equal to the original column. For classes, imputed values are used to not get scewed class sums.
...
unique_lipid_classes <- unique(lipidomicsDF$lipidClass)
for (lipid_class in unique_lipid_classes[!unique_lipid_classes == "PE.PC"]) {
...
lipids_groups <- list(
  ...
  gr_pc = c("PC", "PE.PC"),  # Phosphatidylcholines and ratio
...
Ouliers handled by using pmol_imputed.
For lipid composition, outliers are removed before in the data fed to the dataset
```{r}
# run function test
# Function to prepare data
...
lipid_classes_per_comp <- unique_combinations$lipidClass[!unique_combinations$lipidClass == "PE.PC"]

lipid_compositions <- unique_combinations$lipidComposition[!unique_combinations$lipidClass == "PE.PC"]

# Run function for lipid composition
# PE.PC should not be analysed as a composition, but only on class level
...
unique_lipid_classes <- unique(lipidomicsDF$lipidClass[!lipidomicsDF$lipidClass == "PE.PC"])
...
for (i in 1:nrow(unique_combinations[unique_combinations$lipidClass != c("Cer ratio", "PE.PC"),])) {
  class <- unique_combinations$lipidClass[i]
  comp <- unique_combinations$lipidComposition[i]
