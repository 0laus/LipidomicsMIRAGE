...
hm_data_pvals <- pvals_df[ , c("Pre HED - Post HED (corrected)", "Pre DIWL - Post DIWL (corrected)", 
                               "Pre WLEX - Post WLEX (corrected)", "Pre - Post : HED - DIWL (corrected)",
                               "Pre - Post : HED - WLEX (corrected)", "Pre - Post : DIWL - WLEX (corrected)")]


# Select the specified columns for hm_data.estimates matrix
hm_data_estimates <- estimates_df[ , c("Pre HED - Post HED", "Pre DIWL - Post DIWL", 
                                       "Pre WLEX - Post WLEX", "Pre - Post : HED - DIWL",
                                       "Pre - Post : HED - WLEX", "Pre - Post : DIWL - WLEX")]
...
log2fold_ch_comp <- lipidomicsDF %>%
  ...
  summarize(
    log2FoldChange = log2(mean(pmol_mg[time == "Post"], na.rm = TRUE) / 
...
### Replace changes based on log2change values

### SLOPE
### Calculate means for which to normalise slopes

### Normalise slopes in the data frame



# Convert to matrix
# p-vals
hm_data_pvals_matrix <- as.matrix(hm_data_pvals)
# Estimates
hm_data_estimates_matrix <- as.matrix(hm_data_estimates)
...
## Scatter plots
Scatter plots with insulin sensitivity as colormap. Data points above ULOQ and below LLOQ are marked with triangles for.
```{r}
