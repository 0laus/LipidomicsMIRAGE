...
glycemicControl <- list(
  ...
  c("rd_ss2kg_ffm_insulin_pre", "rd_ss2kg_ffm_insulin_post"),
  c("vo2kg_ffm_pre", "vo2kg_ffm_post"),
  c("hgb_a1c_pre", "hgb_a1c_post"),
  c("homair_pre", "homair_post"),
  c("egp0kg_ffm_pre", "egp0kg_ffm_post"),
  c("egpss1kg_ffm_pre", "egpss1kg_ffm_post"),
  c("egpss2kg_ffm_pre", "egpss2kg_ffm_post"),
  c("age_pre", "age_post")) # Age must appear last in this list!

mitochondria <- list(
  c("at_pmax_p_mrs_pre", "at_pmax_p_mrs_post"),
  c("pcr_p_mrs_pre", "pcr_p_mrs_post"),
  c("max_oxphos_pre", "max_oxphos_post"),
  c("max_ets_pre", "max_ets_post"),
  c("le_pre", "le_post"),
  c("pe_pre", "pe_post"),
  c("age_pre", "age_post")) # Age must appear last in this list!

bodycomposition <- list(
  c("leg_lean_mass_pre", "leg_lean_mass_post"),
  c("fat_mass_dxa_pre", "fat_mass_dxa_post"),
  c("lean_mass_dxa_pre", "lean_mass_dxa_post"),
  c("ffm_dxa_pre", "ffm_dxa_post"),
  c("thigh_sat_g_pre", "thigh_sat_g_post"),
  c("thigh_imat_g_pre", "thigh_imat_g_post"),
  c("percent_imat_pre", "percent_imat_post"),
  c("thigh_muscle_g_pre", "thigh_muscle_g_post"),
  c("abdomen_vat_g_pre", "abdomen_vat_g_post"),
  c("age_pre", "age_post")) # Age must appear last in this list!

blood <-  list(
  c("vldl_pre", "vldl_post"),
  c("ldl_pre", "ldl_post"),
  c("hdl_pre", "hdl_post"),
  c("cholesterol_pre", "cholesterol_post"),
  c("triglycerides_pre", "triglycerides_post"),
  c("plasma_ffabg_pre", "plasma_ffabg_post"),
  c("age_pre", "age_post"))# Age must appear last in this list!
  
  physicalActivity <- list(
  c("mvpa_mins_day_pre", "mvpa_mins_day_post"),
  c("vo2abs_pre", "vo2abs_post"),
  c("vo2kg_ffm_pre", "vo2kg_ffm_post"),
  c("x4m_walk_pre", "x4m_walk_post"),
  c("StepTest_Pre", "StepTest_Post"),
  c("x1rm_pre", "x1rm_post"),
  c("age_pre", "age_post")) # Age must appear last in this list!
...
```{r}
# Glycemic Control
glycemicControl <- c("rd_ss1kg_ffm_insulin_pre", "rd_ss2kg_ffm_insulin_pre", "vo2kg_ffm_pre", "hgb_a1c_pre", "homair_pre", "egp0kg_ffm_pre", "egpss1kg_ffm_pre", "egpss2kg_ffm_pre")

# Mitochondria
mitochondria <- c("at_pmax_p_mrs_pre", "pcr_p_mrs_pre", "max_oxphos_pre", "max_ets_pre", "le_pre", "pe_pre")

bodycomposition <- c("leg_lean_mass_pre", "fat_mass_dxa_pre", "lean_mass_dxa_pre", "ffm_dxa_pre", "thigh_sat_g_pre", "thigh_imat_g_pre", "percent_imat_pre", "thigh_muscle_g_pre", "abdomen_vat_g_pre")

# Blood
blood <- c("vldl_pre", "ldl_pre", "hdl_pre", "cholesterol_pre", "triglycerides_pre", "plasma_ffabg_pre")

# Physical Activity + Age
physicalActivity <- c("mvpa_mins_day_pre", "vo2abs_pre", "vo2kg_ffm_pre", "x4m_walk_pre", "StepTest_Pre", "x1rm_pre", "age_pre")
...
```{r}
endpoint_variable <- c("daily_steps_steps_24hr_pre", "daily_steps_steps_24hr_post")
y_label <- expression("Steps per 24 hours")

mirage_analysis_and_plot <- function(endpoint_variable, y_label)
{
...
plot <- ggplot(mirage_plot_long, aes(
  ...
  geom_line(aes(color = if_else(in_lipidomics == TRUE, "Lipidomics", "MIRAGE"), 
                alpha = if_else(in_lipidomics == TRUE, 1, 0.7)), 
  ...
  geom_point(aes(color = if_else(in_lipidomics == TRUE, "Lipidomics", "MIRAGE"), 
                 alpha = if_else(in_lipidomics == TRUE, 1, 0.7)),  # Adjusted alpha for visibility
             position = position_nudge(x = 0.03)) +
  ...
  geom_line(data = mean_data, aes(x = interaction(group, time, sep = " "), y = mean_mirage, group = group), 
            color = "grey", alpha = 0.8, linewidth = 1, position = position_nudge(x = 0.03)) +
...
print(plot)
  ...
  filename <- paste0(
    "/Users/au309166/Library/CloudStorage/OneDrive-Aarhusuniversitet/Forskning/Projects/LipidomicsMIRAGE/outputs/mirage/",
    endpoint_variable, ".jpg")
...
mirage_analysis_and_plot(endpoint_variable=endpoint_variable, y_label=y_label)
```
...
# Capitalize all
vulc_data$spearman_type <- gsub("all", "ALL", vulc.cor$spearman_type)
...
vulc_plot <- ggplot(vulc_data, aes(x = rho_value, y = -log10(p_value))) +
  ...
  scale_alpha_manual(values = c(`TRUE` = 1, `FALSE` = 0.12)) + # TRUE for significant, FALSE for not
  geom_text_repel(aes(label = ifelse(p_value < 0.05 & abs(rho_value) > threshold_rho, identifier, "")),
                  box.padding = 0.35, point.padding = 0.5,
                  size = 3, segment.color = 'grey50') +
  theme_minimal() +
  labs(title = "Correlation between changes in lipids and changes in clinical endpoints", x = "Spearman rho", y = "-log10 P-value") +
  xlim(-1,1) +
...
# Save the plot
ggsave("/Users/au309166/Library/CloudStorage/OneDrive-Aarhusuniversitet/Forskning/Projects/LipidomicsMIRAGE/outputs/correlations in changes/vulcano/volcano.jpg", plot = vulc_plot, width = 12, height = 8, dpi = 400)
}
...
lipids_groups <- list(
  gr_tag = c("TAG"),  # Triacylglycerols
  gr_dg_ac = c("1,2-DG", "1,3-DG", "AC"),  # Diglycerides and Acyl carnitines
  gr_pc = c("PC", "PE:PC ratio"),  # Phosphatidylcholines and ratio
  gr_pe = c("PE"),  # Phosphatidylethanolamines
  gr_sphingolipids = c("SPM", "SPH", "Cer", "dhCer", "GluCer", "LacCer", "GalCer"),  # Sphingolipids: Sphingomyelins, Sphingosines, Ceramides and derivatives
  gr_lyso = c("LPC", "LPE", "LPG", "LPI", "LPS"),  # Lysophospholipids
  gr_pa_ps = c("PA", "PS"),  # Group combining Phosphatidic acids and Phosphatidylserines
...
mean_data_comp <- filtered_data %>%
  ...
  plot <- ggplot(filtered_data, aes(
    ...
    geom_point(
      ...
      alpha = 0.3, position = position_nudge(x = 0.03) # Down triangle for values below LLOQ
    ) +
...
for (lipid_class in unique_lipid_classes) {
  lipid_compositions_in_class <- unique(lipidomicsDF$lipidComposition[lipidomicsDF$lipidClass == lipid_class])
  for (lipid_composition in lipid_compositions_in_class) {
    plot_lipid(lipidomicsDF, lipid_composition, lipid_class, color_map, detection_limits, est_and_p_lipid_comp$p.vals)
  }
}
```
