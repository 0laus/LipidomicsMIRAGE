...
correlation_of_change <- function(correlation_df, lipidomicsData, mirage_data_long_ch, mirage_variables_of_interest, lipidclass) {
...
######## IN LOOP
for (saturation_types in c("sat_proportion", "unsat_proportion", "unsat_mono_proportion", "unsat_poly_proportion")){
 ...
 for (lipid_class in unique_lipid_classes[unique_lipid_classes != "PE_PC ratio"]) {
   ...
   correlation_df <- prepare_and_combine_data(
     ...
     saturation_col = saturation_types ### <- Make loop over this!
   )
...
# Plot data - un-comment if to be used again
  correlation_change_plot(
    ...
    lipidclass = lipid_class,
    saturation_type = saturation_types
  )
}}
...
## Make correlation stats on change heatmap

## Make crossSection statistics

## Make crossSection vulcano
...
mean_data_comp <- filtered_data %>%
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd(), "/outputs/lipids, pre-post/plot, scatter composition/",
...
plot <- ggplot(pmol_mg_class_sum, aes(
  ...
  # Define the filename and save the plot
  filename <- paste0(
    getwd() ,"/outputs/lipids, pre-post/plot, scatter class/",
    gsub(" ", "_", lipid_class), ".jpg"
  )
  ggsave(filename, plot = plot, device = "jpeg", width = 8, height = 6, dpi = 300)
