...
**Add PE/PC ratio**
Outliers are handled by calculating from imputed values as the PE and PC classes are summed. As they are analysed on a lipid class level, Outlier status is therefore set to FALSE.
...
unique_lipid_classes <- "Cer ratio"# unique(lipidomicsDF$lipidClass[!lipidomicsDF$lipidClass == "PE.PC"])
...
# Dynamic Y-labels
if("PE.PC" %in% filtered_data$lipidClass) {
  y_label <- "PE/PC ratio"
}
else {
  y_label <- switch(unit,
...
label_height <- max(filtered_data[[unit]]) * 0.9
...
for (i in 1:nrow(unique_combinations)) {  
  # is this needed? [unique_combinations$lipidClass != c("Cer ratio", "PE.PC")
  class <- unique_combinations$lipidClass[i]
  comp <- unique_combinations$lipidComposition[i]
