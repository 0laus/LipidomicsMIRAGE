...
prepare_lipid_data_for_plotting <- function(result_data) {
  ...
  rownames(est_df) <- NULL
   ...
   # Keep rows in est_df that are also in the filtered pvals_df based on row names
    est_df <- est_df[rownames(est_df) %in% rownames(pvals_df), ]
...
## Heatmap
```{r}
# Gather log2-fold change and slope
View(est_and_p_lipid_class_and_comp)
