...
Make saturation columns, where first column is defined by the number of double bonds in the first lipid, and the second column is defined by the number of double bonds in the second lipid.
```{r}


lipidomicsDF_sat <- lipidomicsDF %>%
  # Convert factors to character if not already
  mutate(lipidComposition = as.character(lipidComposition)) %>%
  # Create new columns for saturation_1, saturation_2, and count of 'e'
  mutate(
    saturation_1 = sapply(regmatches(lipidComposition, gregexpr("(?<=:)(\\d+)", lipidComposition, perl = TRUE)), `[`, 1),
    saturation_2 = sapply(regmatches(lipidComposition, gregexpr("(?<=:)(\\d+)", lipidComposition, perl = TRUE)), `[`, 2),
  ...
  mutate(
    saturation_2 = ifelse(grepl("di", lipidComposition) & is.na(saturation_2), saturation_1, saturation_2)
  ) %>%
  # Ensure saturation_2 is NA where there is no second match and no "di"
  mutate(saturation_2 = ifelse(is.na(saturation_2), NA, saturation_2))

View(lipidomicsDF_sat)
```
