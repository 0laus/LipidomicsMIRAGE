...
# Create a new data frame (optional but often helpful)
...
# make _change name to get that col
...
ci_vals <- c("", sprintf("[%0.2f, %0.2f]", ci[1], ci[2]),
    ...
    p_vals <- c("", sub("<", "&lt;", format.pval(p, digits=2, eps=0.001)),
      sub("<", "&lt;", format.pval(p_change, digits=2, eps=0.001))
      )
...
mirage_included <- mirage_data %>%
  filter(id %in% lipidomicsDF$id) %>%
  mutate(
    weight_change = weight_post - weight_pre,  # Calculate change in weight
    rd_ss2kg_ffm_insulin_change = rd_ss2kg_ffm_insulin_post - rd_ss2kg_ffm_insulin_pre,
    bmi_change = bmi_post - bmi_pre,
    mvpa_mins_day_change = mvpa_mins_day_post - mvpa_mins_day_pre,
    vo2kg_ffm_change = vo2kg_ffm_post - vo2kg_ffm_pre,
    
    hgb_a1c_change = hgb_a1c_post - hgb_a1c_pre,
    cholesterol_change = cholesterol_post - cholesterol_pre,
    ldl_change = ldl_post - ldl_pre,
    hdl_change = hdl_post - hdl_pre,
    fat_mass_dxa_change = fat_mass_dxa_post - fat_mass_dxa_pre,
    lean_mass_dxa_change = lean_mass_dxa_post - lean_mass_dxa_pre,
    age_change = age_post - age_pre,
    thigh_muscle_g_change = thigh_muscle_g_post - thigh_muscle_g_pre,
    percent_imat_change = percent_imat_post - percent_imat_pre,
    
    
    group = factor(group, levels = c("DIWL", "WLEX", "HED"))  # Example grouping
  )
...
mirage_included$t2d_status <- 
  factor(mirage_included$t2d_status, levels=c(1,0),
         labels=c("Yes", 
                  "No"))
...
label(mirage_included$rd_ss2kg_ffm_insulin_pre) <- "Insulin sensitivityᶜ"
label(mirage_included$mvpa_mins_day_pre) <- "Physical activityᵃ"
...
label(mirage_included$percent_imat_pre) <- "Thigh IMATᵇ"
...
units(mirage_included$rd_ss2kg_ffm_insulin_pre) <- "nmol/min/kg"  
units(mirage_included$mvpa_mins_day_pre) <- "min"
...
units(mirage_included$vo2kg_ffm_pre) <- "ml · kg ffm⁻¹"



# Footnote
footnote <-paste0 ("ᵃ Moderate to vigorous physical activity (MVPA) <br/>", # <br/> used as it's a html table
"ᵇ Intramuscular adipose tissue<br/>",
"ᶜ Glucose rate of disappearance normalized to insulin and fat-free mass")
...
if (name == "age_pre"){
  ...
  summary_stat <- c("Baseline" = mean_sd_pre, "Change" = mean_sd_change)
}
    
    
    # Rename "Mean (SD)2" to "Mean (SD)"
    names(summary_stat)[names(summary_stat) == "Baseline2"] <- "Baseline"
...
result_table <- table1(~ + age_pre + sex_0_male_1_female  + t2d_status + vo2kg_ffm_pre  + weight_pre + bmi_pre +
                          fat_mass_dxa_pre + lean_mass_dxa_pre + thigh_muscle_g_pre + percent_imat_pre + rd_ss2kg_ffm_insulin_pre
                           + hgb_a1c_pre + cholesterol_pre + ldl_pre + hdl_pre | group,
...
# Loop over each lipid class and make plots
for (lipid_class in c("TAG")){ # unique_lipid_classes[!unique_lipid_classes == "Cer ratio"]) {
...
```{r}
View(est_and_p_pvals_lipid_class.outlierRM)
```
...
pmol_mg_class_sum <- filtered_data %>%
  ...
  summarize(
    ...
    .groups = "drop") %>% 
  mutate(sum_pmol_mg = ifelse(lipidClass == "TAG", sum_pmol_mg/1000, sum_pmol_mg))
...
ci_slope_DIWL_WLEX <- result.lipid.pre_post.class.outlierRM[[lipid_class]]$slope_results.ci[result.lipid.pre_post.class.outlierRM[[lipid_class]]$slope_results.ci$group_pairwise =="DIWL - WLEX", c("lower.CL", "upper.CL")]

ci_prepost_HED <- result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci[result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci$contrast =="Pre HED - Post HED", c("lower.CL", "upper.CL")]

ci_prepost_DIWL <- result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci[result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci$contrast =="Pre DIWL - Post DIWL", c("lower.CL", "upper.CL")]

ci_prepost_WLEX <-result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci[result.lipid.pre_post.class.outlierRM[[lipid_class]]$prepost_results.ci$contrast =="Pre WLEX - Post WLEX", c("lower.CL", "upper.CL")]


# Change with a factor 1000 as TAG is reported in mmol and no pmol
if(lipid_class == "TAG"){
  ci_slope_DIWL_WLEX <- ci_slope_DIWL_WLEX/1000
  ci_prepost_HED <- ci_prepost_HED/1000
  ci_prepost_DIWL <- ci_prepost_DIWL/1000
  ci_prepost_WLEX <- ci_prepost_WLEX/1000
}

  # Use switch to simplify the conditional checks
  y_label <- switch(lipid_class,
    "PC.PE" = "PE/PC ratio",
    "TAG" = "Intramuscular triacylglycerole (mmol · mg⁻¹)",
    "1,2-DG" = "1,2 diacylglycerol (pmol · mg⁻¹)",
    switch(unit,
      sum_pmol_mg = expression("Lipid concentration" ~ "(pmol ·" ~ mg^-1 * ")"),
      sum_log_transformed = expression("Log transformed concentration"),
      "Default Label"  # Fallback if none of the above
    )
  )
  

  
  x_pos <- c(.02, .17, 0.3, 0.45, 0.62, 0.77)
  y_lim <- c(0, 200)
...
plot <- ggplot(pmol_mg_class_sum, aes(x = x_pos, y = !!sym(unit), group = interaction(id, group))) +
  geom_line(aes(group = interaction(id, group)), color = "black", alpha = 0.9, linewidth = 0.1) +
  geom_point(alpha = 0.5, size = 0.7) +
  geom_boxplot(aes(group = interaction(group, time)),
    width = 0.03, alpha = 0, outlier.shape = NA,
    position = position_dodge(width = 0.25), color = "black"
  ) +
  geom_line(data = mean_data_class, aes(x = x_pos, y = mean_class, group = group), color = "black", linewidth = 1) +
  labs(y = y_label, x = "") +
  theme_minimal() +
  ylim(y_lim) +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 0, hjust = .5, vjust = 6),
        legend.position = "right", legend.title = element_text(hjust = 0.5, size = 6),
        legend.text = element_text(size = 6), panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(), plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(breaks = x_positions$x_pos, labels=c("DIWL\npre","DIWL\nPost","WLEX\npre","WLEX\nPost","HED\npre","HED\nPost"))


# add significance levels
x_offset <- 0.01
text_size <- 3
y_pos_line_1 <- y_lim[2] * 0.85; y_pos_line_2 <- y_lim[2] * 0.925
y_pos_text_1 <- y_lim[2] * 0.88; y_pos_text_2 <- y_lim[2] *0.955
...
plot<- plot +
  ...
  geom_text(data=tibble(x = c((x_pos[2]-x_pos[1])/2+x_pos[1])), y =c(y_pos_text_1), aes(x=x, y=y),
            label="DIWL time", inherit.aes = FALSE, size = text_size) +
geom_text(data=tibble(x = c((x_pos[4]-x_pos[3])/2+x_pos[3])), y =c(y_pos_text_1), aes(x=x, y=y),
          label="WLEX time", inherit.aes = FALSE,  size = text_size) + 
  geom_text(data=tibble(x = c((x_pos[6]-x_pos[5])/2+x_pos[5])), y =c(y_pos_text_1), aes(x=x, y=y),
            label="HED time", inherit.aes = FALSE, size = text_size) + 
  geom_text(data=tibble(x = c((x_pos[1]+x_pos[4]) /2)), y =c(y_pos_text_2), aes(x=x, y=y),
            label="Time between", inherit.aes = FALSE, size = text_size)

  

# To do:

# Put in 95% CI or P-values dynamically.
    # Ready to get into figure!

# Implement in loop

# Inputs lipidclass, y-lim, p-value / CI; y-label
...
print(plot)
  # Define the filename and save the plot
  filename <- paste0(
    getwd() ,"/outputs/figures/figure 1/", save_subfolder, "/",
    gsub(" ", "_", lipid_class), ".jpg"
  )
  ggsave(filename, plot = plot, device = "jpeg", width = 5, height = 6, dpi = 300)
...
for (lipid_class in c("TAG")){  #unique_lipid_classes) {
