...
long_format_p_adj.non <- correlation_values_ch_composition %>%
...
long_format_cor <- correlation_values_ch_composition %>%
...
lipids_groups <- list(
  ...
  gr_spm_sph = c("SPM", "SPH"),  # Sphingomyelins, Sphingosines
  gr_cer = c("Cer", "dhCer", "GluCer", "LacCer", "GalCer"),  # Ceramides and derivatives
  gr_lyso = c("LPC", "LPE", "LPG", "LPI", "LPS")  # Lysophospholipids
)


# Define function to filter lipids based on group names
filter_lipid_group_base <- function(data_frame, lipid_group) {
  pattern <- paste0("^(", paste(lipid_group, collapse = "|"), ")")
  data_frame[grepl(pattern, data_frame$lipid), ]
}
...
spread_and_convert_p_vals_matrix <- function(df, unitName) {
  result_matrix <- df %>%
          ...
          cell_fun = function(j, i, x, y, width, height, fill) {
              if (!is.na(p_val_matrix[i, j])) {
                  if (p_val_matrix[i, j] < pLims$p1) {
                      grid.text("+++", x, y, gp = gpar(col = "black", fontsize = 8, fontface = "bold"), just = "center")
...
4) Organize data into two dataframes: both with rows as lipids and columns as variables of interest
```{r}
mirage_ch_vars # The data to use exept for age
```
