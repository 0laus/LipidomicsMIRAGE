...
Make into long format based on defined interesting variables
...
```{r}
# Make column with Z-transformed data
lipidomicsDF <- lipidomicsDF %>% mutate(pmol_mg_znorm = scale(pmol_mg))
```

Sort data for future analysis
...
### Correlation statistics (function)
...
### Run functions: Prepare, stat, plot
...
plot_lipid <- function(lipidomicsDF, lipid_composition, lipid_class, color_map, detection_limits, p_val_df) {
  ...
  filename <- paste0(
    "/Users/au309166/Library/CloudStorage/OneDrive-Aarhusuniversitet/Forskning/Projects/LipidomicsMIRAGE/outputs/plot, scatter composition/",
...
plot_lipid_class <- function(lipidomicsDF, lipid_class, detection_limits, p_val_df) {
  ...
  plot <- ggplot(filtered_data, aes(
    ...
    y = pmol_mg_normalized, group = interaction(id, group)
  )) +
    ...
    labs(title = lipid_class, subtitle = annotation_pvals_slope, y = "Lipid concentraction, min-max normalised", x = "") +
    # Insert P-values pre post
    annotate("text",
  ...
  # Define the filename and save the plot
  filename <- paste0(
    "/Users/au309166/Library/CloudStorage/OneDrive-Aarhusuniversitet/Forskning/Projects/LipidomicsMIRAGE/outputs/plot, scatter class/",
