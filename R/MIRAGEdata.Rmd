...
```{r}
# Glycemic Control
glycemicControl <- c("rd_ss2kg_ffm_insulin_pre", "hgb_a1c_pre")

# Mitochondria
mitochondria <- c("pcr_p_mrs_pre", "max_oxphos_pre")

bodycomposition <- c("percent_imat_pre")

# Blood
blood <- c("vldl_pre", "ldl_pre", "hdl_pre", "cholesterol_pre", "triglycerides_pre", "plasma_ffabg_pre")

# Physical activity + Age
physicalActivity <- c("tdpa_pre", "vo2kg_ffm_pre", "age_pre")#, "vo2abs_pre", "vo2kg_ffm_pre", "age_pre")
...
label(mirage_included$tdpa_pre) <- "Physical activityᵃ"
...
units(mirage_included$rd_ss2kg_ffm_insulin_pre) <- "mg/kg/min/ins"
...
# Footnote
footnote <-paste0 ("ᵃ Daily physical activity > 1.5 MET <br/>", # <br/> used as it's a html table
                   "ᵇ Intramuscular adipose tissue<br/>",
                   "ᶜ Glucose infusion rate normalized to fat-free mass and blood insulin")
...
result_table <- table1(~ + age_pre + sex_0_male_1_female  + t2d_status  + 
                         bmi_pre + weight_pre + 
                         fat_mass_dxa_pre + lean_mass_dxa_pre + thigh_muscle_g_pre + percent_imat_pre + rd_ss2kg_ffm_insulin_pre + vo2kg_ffm_pre + tdpa_pre +
...
ggsave(paste0(getwd(), "/outputs/figures/baseline correlations/waterfall/", clinical_var[[1]], ".svg"), plot, device = "pdf", height = 80, width = 55 , units = "mm", bg = "transparent")
...
clinical_var <- list(
  ...
  c("rd_ss2kg_ffm_insulin_ch", bquote(bold("Glucose infusion rate")), .7),
...
ht_opt$ROW_ANNO_PADDING <- unit(0, "mm")
  ...
  if (p_val_matrix[i, j] < pLims$p1) {
   ...
   savelocation_svg <- paste0(getwd(), "/outputs/figures/waterfall/heatmaps/", clin_var[[1]], "/", lipids_groups[[i]][2], ".svg")
...
lipids_groups <- list(
  gr_tag_ac = c("TAG", "Triacylglycerol", 0.65, 2.5, FALSE, "row_title"),
  gr_1_2_dg = c("1,2-DG", "1,2-diacylglycerol", 0.82, 2.9, FALSE, "row_title"),
  gr_1_3_dg = c("1,3-DG", "1,3-diacylglycerol", 0.82, 2.9, FALSE, "row_title"),
  gr_ac = c("AC", "Acylcarnitine", 0.65, 1.1, FALSE, "row_title"),
  gr_cer = c("Cer", "Ceramide", 0.65, .6, FALSE, "row_title"),
  ...
  gr_spm = c("SPM", "Sphingomyelin", 0.65, .7, FALSE, "row_title"),
  ...
  gr_pg = c("PG", "Phosphatidylglycerol", 0.82, 2.5, FALSE, "row_title"),
  gr_ps = c("PS", "Phosphatidylserine", 0.82, 2.3, FALSE, "row_title"),
  gr_pi = c("PI", "Phosphatidylinositol", 0.82, 2.3, FALSE, "row_title"),
  gr_lpc = c("LPC", "Lysophosphatidylcholine", 0.65, 1.2, FALSE, "col_title"),
...
clinical_var <- list(
  ...
  c("rd_ss2kg_ffm_insulin_ch", bquote("Glucose infusion rate"), .7),
...
if (plot_lipids[i] %in% c("TAG", "PE", "PC", "PI")) {
 ...
 plot <- ggplot(correlation_df, aes(x = pmol_mg_ch_id, y = !!sym(var_name_ch), shape = group, fill = gender)) +
      ...
      geom_smooth(aes(group = 1), method = "lm", se = TRUE, color = "black", alpha = 0.3, linewidth = 0.5) +
  labs(
    x = x_label,
...
# Selected clinical  
clinical_vars <- list(
  c("rd_ss2kg_ffm_insulin_pre", "rd_ss2kg_ffm_insulin_post", bquote(atop(NA, atop(textstyle(bold("Glucose infusion rate")), textstyle(Delta * " " * "mg ·(min·kgFFM·ins)"^{-1}))))),
  ...
  c("percent_imat_pre", "percent_imat_post", bquote(atop(NA, atop(textstyle(bold(Thigh IMAT)), textstyle(Delta * " " * "%")))))
...
plot <- ggplot(corr_ordered, aes(lipid, spearman_rho_all)) +
  ...
  geom_text(aes(label = significance,
            ...
            size = 4) +      # Enlarge the size of asterisks
  coord_flip() +
  labs(x = "Predictor Variable", y = "Spearman's ρ", title = clinical_var[[2]]) +
...
ggsave(paste0(getwd(), "/outputs/figures/waterfall/", clinical_var[[1]], ".svg"), plot, device = "pdf", height = 80, width = 55 , units = "mm")
...
clinical_var <- list(
  ...
  c("rd_ss2kg_ffm_insulin_pre", bquote(bold("Glucose infusion rate")), .7),
...
if (is.na(hm_matrix[i, j])) {
  ...
  if (p_val_matrix[i, j] < pLims$p1) {
   ...
   savelocation_svg <- paste0(getwd(), "/outputs/figures/baseline correlations/waterfall/heatmaps/", clin_var[[1]], "/", lipids_groups[[i]][2], ".svg")
   dir.create(dirname(savelocation_svg), recursive = TRUE, showWarnings = FALSE)
...
clinical_var <- list(
  ...
  c("rd_ss2kg_ffm_insulin_pre", bquote("Glucose infusion rate"), .7),
  c("percent_imat_pre", bquote("Thigh IMAT proportion"), .7),
...
# Save the grid of plots in SVG format
ggsave(paste0(getwd(), "/outputs/figures/baseline correlations/grid_plots_classes/", save_name, ".svg"), plot_grid, device = "pdf", height = save_size[1], width = save_size[2] , units = "mm")
}

# Selected clinical  
clinical_vars <- list(
  c("rd_ss2kg_ffm_insulin_pre", bquote(atop(NA, atop(textstyle(bold("Glucose infusion rate")), textstyle( "mg ·(min·kgFFM·ins)"^{-1}))))),
...
generate_heatmap <- function(hm_matrix, p_val_matrix, colorMap, pLims, hm_title, hm_unit) {
  ...
  Heatmap(hm_matrix,
          ...
          cell_fun = function(j, i, x, y, width, height, fill) {
            ...
            if (!is.na(p_val_matrix[i, j])) {
              if (p_val_matrix[i, j] < pLims$p1) {
                grid.text("+++", x, y, gp = gpar(col = "black", fontsize = 5, fontface = "plain"), just = "center")
              } else if (p_val_matrix[i, j] < pLims$p2) {
                grid.text("++", x, y, gp = gpar(col = "black", fontsize = 5, fontface = "plain"), just = "center")
              } else if (p_val_matrix[i, j] < pLims$p3) {
                grid.text("+", x, y, gp = gpar(col = "black", fontsize =5, fontface = "plain"), just = "center")
