...
mvpa_mins_day_change = (mvpa_mins_day_post - mvpa_mins_day_pre) / mvpa_mins_day_pre * 100,
...
label(mirage_included$rd_ss2kg_ffm_insulin_pre) <- "GIR/Iᶜ"
...
"ᶜ Glucose infusion rate normalized to insulin and fat-free mass")
...
result_table <- table1(~ + age_pre + sex_0_male_1_female  + t2d_status + vo2kg_ffm_pre  + 
                         bmi_pre + weight_pre + 
                          fat_mass_dxa_pre + lean_mass_dxa_pre + thigh_muscle_g_pre + percent_imat_pre + rd_ss2kg_ffm_insulin_pre
                           + hgb_a1c_pre + triglycerides_pre + cholesterol_pre + ldl_pre + hdl_pre | group,
...
### Figure 1
 TO DO
- Make plot store plot objects -> Combine into a single plot
```{r}
plot_lipid_class <- function(lipidomicsDF, lipid_class, p_val_df, unit, y_limit, y.labels, save_subfolder) {

  
   # Filter data based on lipid class and available time
  filtered_data <- lipidomicsDF %>%
    filter(lipidClass == lipid_class & !is.na(time))
# if(lipid_class == "GalCer"){
#   browser()
# }
...
# Get p-values
p_slope_DIWL_WLEX <- p_val_df[[lipid_class]]$slope_results[p_val_df[[lipid_class]]$slope_results$contrast =="Pre - Post : DIWL - WLEX", c("p.value")]

p_prepost_HED <- p_val_df[[lipid_class]]$prepost_results[p_val_df[[lipid_class]]$prepost_results$contrast =="Pre HED - Post HED", c("p.value")]

p_prepost_DIWL <- p_val_df[[lipid_class]]$prepost_results[p_val_df[[lipid_class]]$prepost_results$contrast =="Pre DIWL - Post DIWL", c("p.value")]

p_prepost_WLEX <-p_val_df[[lipid_class]]$prepost_results[p_val_df[[lipid_class]]$prepost_results$contrast =="Pre WLEX - Post WLEX", c("p.value")]
...
p_slope_DIWL_WLEX <- format_p_value(p_slope_DIWL_WLEX)
p_prepost_HED<- format_p_value(p_prepost_HED)
p_prepost_DIWL <- format_p_value(p_prepost_DIWL)
p_prepost_WLEX <- format_p_value(p_prepost_WLEX)
...
plot <- ggplot(pmol_mg_class_sum, aes(x = x_pos, y = !!sym(unit), group = interaction(id, group))) +
  ...
  theme_minimal() +
  ylim(y_lim) +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = .9, vjust = 2.4, size = 6),
        legend.position = "right", legend.title = element_text(hjust = 0.5, size = 6),
        panel.grid.major.x = element_blank(), 
        plot.margin = margin(t = 0, r = 5, b = 6, l = 2, unit = "pt"),
...
geom_text(data=tibble(x = c((x_pos[4]-x_pos[3])/2+x_pos[3])), y =c(y_pos_text_1), aes(x=x, y=y),
  ...
  geom_text(data= tibble(x = c((x_pos[1]+x_pos[2])/2, (x_pos[3]+x_pos[4])/2, (x_pos[5]+x_pos[6])/2)), y =c(0), aes(x=x, y=y),
            label=c("DIWL", "WLEX", "HED"), inherit.aes = FALSE, vjust  = 4.5, fontface="bold", size = 2.5)
...
#     gsub(" ", "_", lipid_class), ".jpg"
#   )
#   ggsave(filename, plot = plot, device = "jpeg", width = 3, height = 4, dpi = 300)
# 
#   cat("Plot saved as:", filename, "\n")
#   
}
...
y.labels <- list(
  "TAG" = "Triacylglycerol (nmol · mg⁻¹)",
  "1,2-DG" = "1,2 diacylglycerol (pmol · mg⁻¹)",
  "1,3-DG" = "1,3 diacylglycerol (pmol · mg⁻¹)",
  "AC" = "Acylcarnitine (nmol · mg⁻¹)",
  "Cer" = "Ceramide (pmol · mg⁻¹)",
  "dhCer" = "Dihydroceramide (pmol · mg⁻¹)",
  "GalCer" = "Galactosylceramide (pmol · mg⁻¹)",
  "GluCer" = "Glucosylceramide (pmol · mg⁻¹)",
  "LacCer" = "Lactosylceramide (pmol · mg⁻¹)",
  "LPC" = "Lysophosphatidylcholine (pmol · mg⁻¹)",
  "LPE" = "Lysophosphatidylethanolamine (pmol · mg⁻¹)",
  "LPG" = "Lysophosphatidylglycerol (pmol · mg⁻¹)",
  "LPI" = "Lysophosphatidylinositol (pmol · mg⁻¹)",
  "LPS" = "Lysophosphatidylserine (pmol · mg⁻¹)",
  "PA" = "Phosphatidic acid (pmol · mg⁻¹)",
  "PC" = "Phosphatidylcholine (nmol · mg⁻¹)",
  "PE" = "Phosphatidylethanolamine (nmol · mg⁻¹)",
  "PG" = "Phosphatidylglycerol (pmol · mg⁻¹)",
  "PI" = "Phosphatidylinositol (pmol · mg⁻¹)",
  "PS" = "Phosphatidylserine (pmol · mg⁻¹)",
  "SPH" = "Sphingosine (pmol · mg⁻¹)",
  "SPM" = "Sphingomyelin (pmol · mg⁻¹)",
  "Cer ratio" = "Ceramide 16:0 / ceramide 24:0",
  "PC.PE" = "Phophoethanolamine / phosphatidylcholine"
)
...
for (lipid_class in unique_lipid_classes) {
  list_of_plots[[lipid_class]] <- plot_lipid_class(lipidomicsDF = lipidomicsDF,
                   ...
                   y_limit = y.lims[[lipid_class]],
                   y.labels = y.labels [[lipid_class]],
                   unit = "sum_pmol_mg",
                   save_subfolder = "")
}

acylglycerole_row.1 <- list_of_plots[["TAG"]] | 
...
filename <- paste0( getwd() ,"/outputs/figures/figure 1/", "tag_ac.jpg")
ggsave(filename, plot = plot_layout_1, device = "jpeg", width = 9, height = 4, dpi = 300)

plot_layout_2 <- Sphingolipids_row.1 / Sphingolipids_row.2 +
  plot_annotation(tag_levels = 'A')
filename <- paste0( getwd() ,"/outputs/figures/figure 1/", "sphingolipids.jpg")
ggsave(filename, plot = plot_layout_2, device = "jpeg", width = 9, height = 8, dpi = 300)

plot_layout_3 <- phospholipid_row.1 / phospholipid_row.2 +
  plot_annotation(tag_levels = 'A')
filename <- paste0( getwd() ,"/outputs/figures/figure 1/", "phospholipids.jpg")
ggsave(filename, plot = plot_layout_3, device = "jpeg", width = 9, height = 8, dpi = 300)

plot_layout_4 <- lysophospholipids_row.1 +
  plot_annotation(tag_levels = 'A')
filename <- paste0( getwd() ,"/outputs/figures/figure 1/", "lysophospholipids.jpg")
ggsave(filename, plot = plot_layout_4, device = "jpeg", width = 9, height = 4, dpi = 300)
