...
ci_vals <- c("", sprintf("[%0.2f, %0.2f]", ci[1], ci[2]),
    ...
    p_vals <- c("", sub("<", "&lt;", format.pval(p, digits=3, eps=0.001)),
      sub("<", "&lt;", format.pval(p_change, digits=3, eps=0.001))
      )
...
result_table <- table1(~ sex_0_male_1_female + t2d_status+ age_pre + weight_pre + bmi_pre + 
                          fat_mass_dxa_pre + lean_mass_dxa_pre + thigh_muscle_g_pre + percent_imat_pre + rd_ss2kg_ffm_insulin_pre + 
                          mvpa_mins_day_pre + hgb_a1c_pre + cholesterol_pre + ldl_pre + hdl_pre | group,
                       data = mirage_included,
                       render = custom_render,
                       footnote = footnote,
                       overall = FALSE,
                       extra.col=list(`95% CI`=pvalue),
                       extra.col.pos = c(3))
